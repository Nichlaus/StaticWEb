		...
		<div class="card-body">
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT t.*,p.name as pname,p.start_date,p.status as pstatus, p.end_date,p.id as pid FROM Assignment_list t inner join Course_list p on p.id = t.Course_id $where order by p.name asc");
					while($row= $qry->fetch_assoc()):
						$trans = get_html_translation_table(HTML_ENTITIES,ENT_QUOTES);
						unset($trans["\""], $trans["<"], $trans[">"], $trans["<h2"]);
						$desc = strtr(html_entity_decode($row['description']),$trans);
						$desc=str_replace(array("<li>","</li>"), array("",", "), $desc);
						$tprog = $conn->query("SELECT * FROM Assignment_list where Course_id = {$row['pid']}")->num_rows;
		                $cprog = $conn->query("SELECT * FROM Assignment_list where Course_id = {$row['pid']} and status = 3")->num_rows;
						$prog = $tprog > 0 ? ($cprog/$tprog) * 100 : 0;
		                $prog = $prog > 0 ?  number_format($prog,2) : $prog;
		                $prod = $conn->query("SELECT * FROM user_productivity where Course_id = {$row['pid']}")->num_rows;
					...
					<tr>
						...
						<td>
							<p><b><?php echo ucwords($row['Assignment']) ?></b></p>
							...
							<button type="button" class="btn btn-default btn-sm btn-flat border-info wave-effect text-info dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
		                      Action
			                    ...
			                    <div class="dropdown-menu" style="">
			                      <a class="dropdown-item new_productivity" data-pid = '<?php echo $row['pid'] ?>' data-tid = '<?php echo $row['id'] ?>'  data-Assignment = '<?php echo ucwords($row['Assignment']) ?>'  href="javascript:void(0)">Progress</a>
								</div>
						</td>
					</tr>	
				<?php endwhile; ?>
...
<script>
	...
	$('.new_productivity').click(function(){
		uni_modal("<i class='fa fa-plus'></i> New Progress for: "+$(this).attr('data-Assignment'),"manage_progress.php?pid="+$(this).attr('data-pid')+"&tid="+$(this).attr('data-tid'),'large')
	})
	})
