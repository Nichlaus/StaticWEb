...
Class Action {
	...
	function save_Assignment(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO Assignment_list set $data");
		}else{
			$save = $this->db->query("UPDATE Assignment_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_Assignment(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM Assignment_list where id = $id");
		if($delete){
			return 1;
		}
	}
