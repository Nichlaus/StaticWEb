...
<div class="container-fluid">
	<form action="" id="manage-progress">
		...
		<input type="hidden" name="Course_id" value="<?php echo isset($_GET['pid']) ? $_GET['pid'] : '' ?>">
		<div class="col-lg-12">
			<div class="row">
				<div class="col-md-5">
					<?php if(!isset($_GET['tid'])): ?>
					 <div class="form-group">
		              ...
		              <select class="form-control form-control-sm select2" name="Assignment_id">
		              	<option></option>
		              	<?php 
		              	$Assignments = $conn->query("SELECT * FROM Assignment_list where Course_id = {$_GET['pid']} order by Assignment asc ");
		              	while($row= $Assignments->fetch_assoc()):
		              	?>
		              	<option value="<?php echo $row['id'] ?>" <?php echo isset($Assignment_id) && $Assignment_id == $row['id'] ? "selected" : '' ?>><?php echo ucwords($row['Assignment']) ?></option>
		              	<?php endwhile; ?>
		              </select>
		            </div>
		            <?php else: ?>
					<input type="hidden" name="Assignment_id" value="<?php echo isset($_GET['tid']) ? $_GET['tid'] : '' ?>">
		            <?php endif; ?>
					<div class="form-group">
				...
				<div class="col-md-7">
					<div class="form-group">
						<label for="">Assignment Work</label>
