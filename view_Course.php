...
$tprog = $conn->query("SELECT * FROM Assignment_list where Course_id = {$id}")->num_rows;
$cprog = $conn->query("SELECT * FROM Assignment_list where Course_id = {$id} and status = 3")->num_rows;
$prog = $tprog > 0 ? ($cprog/$tprog) * 100 : 0;
$prog = $prog > 0 ?  number_format($prog,2) : $prog;
$prod = $conn->query("SELECT * FROM user_productivity where Course_id = {$id}")->num_rows;
...
<div class="col-lg-12">
			...
			<div class="callout callout-info">
				<div class="col-md-12">
					<div class="row">
						<div class="col-sm-6">
							<dl>
								<dt><b class="border-bottom border-primary">Course Name</b></dt>
						...
						<div class="col-md-6">
							...
							<dl>
								<dt><b class="border-bottom border-primary">Lecturer</b></dt>
								<dd>
	...
	<div class="row">
		...
		<div class="col-md-8">
			<div class="card card-outline card-primary">
				<div class="card-header">
					<span><b>Assignment List:</b></span>
					<?php if($_SESSION['login_type'] != 3): ?>
					<div class="card-tools">
						<button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="new_Assignment"><i class="fa fa-plus"></i> New Assignment</button>
					</div>
				<?php endif; ?>
				</div>
				<div class="card-body p-0">
					...
					<table class="table table-condensed m-0 table-hover">
						...
						<tbody>
							<?php 
							$i = 1;
							$Assignments = $conn->query("SELECT * FROM Assignment_list where Course_id = {$id} order by Assignment asc");
							...
							?>
								<tr>
			                        ...
			                        <td class=""><b><?php echo ucwords($row['Assignment']) ?></b></td>
			                        ...
			                        <td class="text-center">
					                    ...
					                    <div class="dropdown-menu" style="">
					                      <a class="dropdown-item view_Assignment" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>"  data-Assignment="<?php echo $row['Assignment'] ?>">View</a>
					                      <div class="dropdown-divider"></div>
					                      <?php if($_SESSION['login_type'] != 3): ?>
					                      <a class="dropdown-item edit_Assignment" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>"  data-Assignment="<?php echo $row['Assignment'] ?>">Edit</a>
					                      <div class="dropdown-divider"></div>
					                      <a class="dropdown-item delete_Assignment" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">Delete</a>
					                  <?php endif; ?>
					                    </div>
									</td>
		                    	</tr>
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				<div class="card-header">
					<b>Members Progress</b>
					<div class="card-tools">
						<button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="new_productivity"><i class="fa fa-plus"></i> Add Progress</button>
					</div>
				</div>
				<div class="card-body">
					<?php 
					$progress = $conn->query("SELECT p.*,concat(u.firstname,' ',u.lastname) as uname,u.avatar,t.Assignment FROM user_productivity p inner join users u on u.id = p.user_id inner join Assignment_list t on t.id = p.Assignment_id where p.Course_id = $id order by unix_timestamp(p.date_created) desc ");
						...
						<div class="post">
		                      ...
		                      <div class="user-block">
								  ...
								  <div class="dropdown-menu">
								  	<a class="dropdown-item manage_progress" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>"  data-Assignment="<?php echo $row['Assignment'] ?>">Edit</a>
			                      	<div class="dropdown-divider"></div>
		                        ...
		                        <span class="username">
		                          <a href="#"><?php echo ucwords($row['uname']) ?>[ <?php echo ucwords($row['Assignment']) ?> ]</a>
		                        </span>
...
<script>
	$('#new_Assignment').click(function(){
		uni_modal("New Assignment For <?php echo ucwords($name) ?>","manage_Assignment.php?pid=<?php echo $id ?>","mid-large")
	})
	$('.edit_Assignment').click(function(){
		uni_modal("Edit Assignment: "+$(this).attr('data-Assignment'),"manage_Assignment.php?pid=<?php echo $id ?>&id="+$(this).attr('data-id'),"mid-large")
	})
	$('.view_Assignment').click(function(){
		uni_modal("Assignment Details","view_Assignment.php?id="+$(this).attr('data-id'),"mid-large")
	})
